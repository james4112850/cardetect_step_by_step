...
def main(input_dir: Optional[str] = None, output_dir: Optional[str] = None, weights_path: Optional[str] = None) -> None:
    ...
    if input_dir is None:
        # 從 d_crop_plate 讀取
        input_dir = os.path.join(project_root, "result", "d_crop_plate")
    if output_dir is None:
        output_dir = os.path.join(project_root, "result", "e_characters")
    if weights_path is None:
        weights_path = os.path.join(os.path.dirname(__file__), "characters.pt")

    # 嚴格檢查輸入資料夾來源
    expected_input = os.path.join(project_root, "result", "d_crop_plate")
    ...
    # 僅處理前一步輸出的車牌裁剪結果
    image_paths = [p for p in list_images(input_dir) if "_d(crop_plate)" in basename_no_ext(p)]
    if not image_paths:
