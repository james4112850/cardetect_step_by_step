...
def main(input_dir: Optional[str] = None, output_dir: Optional[str] = None, weights_path: Optional[str] = None) -> None:
    ...
    if input_dir is None:
        input_dir = os.path.join(project_root, "stepbystep", "d_crop_plate")
    if output_dir is None:
        output_dir = os.path.join(project_root, "stepbystep", "e_characters")
    if weights_path is None:
        weights_path = os.path.join(os.path.dirname(__file__), "characters.pt")

    # 嚴格檢查輸入資料夾來源
    expected_input = os.path.join(project_root, "stepbystep", "d_crop_plate")
